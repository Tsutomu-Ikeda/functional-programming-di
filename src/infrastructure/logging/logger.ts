...
export class RequestScopedLogger implements Logger {
  ...
  warn = (message: string, context?: object): IO.IO<void> => () => {
    this.log({
      timestamp: new Date(),
      level: 'warn',
      message,
  ...
  debug = (message: string, context?: object): IO.IO<void> => () => {
    this.log({
      timestamp: new Date(),
      level: 'debug',
      message,
  ...
  private log(entry: LogEntry): void {
    if (this.config.format === 'json') {
      ...
      console.log(`[${entry.timestamp.toISOString()}] ${entry.level.toUpperCase()} [${entry.requestId}] ${entry.message}${contextStr}${errorStr}`);
    }
  }
}
...
export class SingletonLogger implements Logger {
  ...
  private log(entry: LogEntry): void {
    if (this.config.format === 'json') {
      ...
      const contextStr = entry.context ? ` ${JSON.stringify(entry.context)}` : '';
      const errorStr = entry.error ? ` Error: ${entry.error.message}` : '';
      console.log(`[${entry.timestamp.toISOString()}] ${entry.level.toUpperCase()} ${entry.message}${contextStr}${errorStr}`);
    }
  }
}
