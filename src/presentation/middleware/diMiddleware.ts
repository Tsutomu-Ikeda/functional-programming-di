...
const createRequestContext =
  (req: Request): IO.IO<RequestContext> =>
  () => ({
    requestId: uuidv4(),
    startTime: new Date(),
    metadata: {
...
const createScopedContainer = (
  globalContainer: DIContainer,
  context: RequestContext,
): TE.TaskEither<MiddlewareError, ScopedContainer> =>
  pipe(
    TE.tryCatch(
...
const attachToRequest =
  (req: Request, context: RequestContext, scopedContainer: ScopedContainer): IO.IO<AuthenticatedRequest> =>
  () => {
...
const handleMiddlewareError =
  (error: MiddlewareError, next: NextFunction): IO.IO<void> =>
  () => {
    console.error('DI Middleware error:', error);

    const expressError = new Error(
      error._tag === 'ContainerScopeError'
        ? 'Failed to create request scope'
        : error._tag === 'LoggerError'
          ? 'Failed to initialize request logging'
          : error._tag === 'RequestContextError'
...
export const createDIMiddlewareFP =
  (globalContainer: DIContainer) =>
