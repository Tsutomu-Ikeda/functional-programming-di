...
describe('Integration Tests', () => {
  ...
  describe('End-to-End User Creation Flow', () => {
    ...
    it('should fail fast on validation errors without calling external services', async () => {
      ...
      if (E.isLeft(result) && result.left._tag === 'ValidationError') {
        ...
        expect(result.left.errors).toEqual(
    ...
    it('should handle database errors appropriately', async () => {
      ...
      expect(E.isLeft(result)).toBe(true)
      if (E.isLeft(result)) {
        expect(result.left).toEqual(dbError)
      }
  ...
  describe('Error Recovery Scenarios', () => {
    it('should handle concurrent user creation attempts', async () => {
      ...
      if (E.isLeft(result2) && result2.left._tag === 'ValidationError') {
        expect(result2.left.errors).toContainEqual({
          field: 'email',
